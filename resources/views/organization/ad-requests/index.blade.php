...
@section('page-title', 'Заявки на рекламу')
@section('content')
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <p class="text-sm text-text-secondary">Все ваши заявки на размещение рекламы на экранах ТРЦ</p>
        <a href="{{ route('organization.ad-requests.create') }}" class="px-4 py-2 bg-accent text-white rounded-lg text-sm font-medium hover:bg-accent-hover transition-colors">
            + Новая заявка
        </a>
    </div>
            ...
            <table class="min-w-full divide-y divide-surface-border">
                <thead class="bg-surface">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-text-secondary uppercase">Статус заявки</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-text-secondary uppercase">Статус видео</th>
                ...
                <tbody class="divide-y divide-surface-border">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 text-sm text-text-secondary">{{ $booking->media->width_px ?? 0 }} x {{ $booking->media->height_px ?? 0 }}</td>
                        ...
                        <td class="px-6 py-4">
                            @php $mediaStatus = $booking->media?->status?->name ?? '—'; @endphp
                            <span class="inline-flex rounded-full px-2 py-1 text-xs font-medium
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-8 text-center text-text-secondary">
                            У вас пока нет заявок на рекламу.
                            <a href="{{ route('organization.ad-requests.create') }}" class="text-accent hover:text-accent-hover">Подать первую заявку</a>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
