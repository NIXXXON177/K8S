                ...
                <nav class="flex items-center gap-2">
                    @auth
                        @if(auth()->user()->isAdmin())
                            <a href="{{ route('admin.dashboard') }}" class="px-4 py-2 rounded-lg text-sm font-medium text-text-secondary transition-colors hover:text-white hover:bg-surface-lighter">Админ-панель</a>
                        @elseif(auth()->user()->isTenant())
                            <a href="{{ route('organization.dashboard') }}" class="px-4 py-2 rounded-lg text-sm font-medium text-text-secondary transition-colors hover:text-white hover:bg-surface-lighter">Личный кабинет</a>
                        @endif
                        <form action="{{ route('logout') }}" method="POST" class="inline">
                            @csrf
                            <button type="submit" class="px-4 py-2 rounded-lg text-sm font-medium text-text-muted transition-colors hover:text-white hover:bg-surface-lighter">Выйти</button>
                        </form>
                    @else
                        <a href="{{ route('login') }}" class="px-4 py-2 rounded-lg text-sm font-medium text-text-secondary transition-colors hover:text-white hover:bg-surface-lighter">Войти</a>
                        <a href="{{ route('register') }}" class="px-4 py-2 border border-accent text-accent hover:bg-accent hover:text-white rounded-lg text-sm font-semibold transition-colors">Регистрация</a>
                    @endauth
                </nav>
            </div>
        </div>
    ...
    <footer class="bg-surface border-t border-surface-border mt-16">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div class="text-center space-y-2">
                <div class="text-sm text-text-muted">&copy; {{ date('Y') }} ТРЦ «Европейский». Система управления мероприятиями и рекламными зонами.</div>
                <div class="text-xs"><span class="shiny-text">Разработчик: Кудряшов Никола, ИСП-029</span></div>
            </div>
        </div>
    </footer>
